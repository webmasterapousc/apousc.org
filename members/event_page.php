...
<?php /*
			if ( !is_null($req_event_info['location_event'])){
$query_location = mysql_query("SELECT * FROM `location` WHERE location_id = ". $req_event_info['location_event']) OR die(mysql_error());
$result_location = mysql_fetch_array($query_location);						
			echo "Located At <span class='tile'>";
			if (!is_null($req_event_info['location'])){
				echo $result_location['location_short'] . "</span> (";};
			echo $result_location['location_full'];
			if (!is_null($req_event_info['location'])){echo ")";};};
      */ ?>
...
if (mysql_num_rows($result) > 0) {
	...
	if (($session->isOfficer())) {	echo "\t\t\t\t\t\t<th scope=\"col\" colspan=\"1\"><a href=event_page.php?eventid=$req_event&order=weight&amp;sort=".$ascdesc."><span class='bright'>Weight</span></th>\n";
	}
	if (
			//($session->position == 0) ||    this makes the column but not anything under it
			...
			//LOOK HERE U TWAT :D this is where you give permission to take people off events
			//change the username == "cguan" to the person with that position
			($session->username == "ecoughla" && ($req_event_info['type']==5 || $req_event_info['type']==9 || $req_event_info['type']==14))  ||		//pledgemaster
			($session->username == "chucamer" && ($req_event_info['type']==7 || $req_event_info['type']==0))  || 		// vp service
			
			($session->username == "lov" && ($req_event_info['type']==4 || $req_event_info['type']==14))  ||		//vp membership
			($session->username == "tylerase" && ($req_event_info['type']==4 || $req_event_info['type']==14))  ||		//vp membership
			($session->username == "zhangkat" && $req_event_info['type']==1)  ||		//vp fellowship
			($session->username == "aschuang" && $req_event_info['type']==2)  ||			//vp finance
			($session->username == "lee975" && $req_event_info['type']==2)  ||		//vp finance
			($session->username == "elwalburger" && $req_event_info['type']==8)  ||	//vp comm
			($session->username == "wangyiha" && $req_event_info['type']== 3)  ||	//ic chair
			// ($session->username == "karendch" && $req_event_info['type']==9)  ||	//sectionals chair
			// ($session->username == "jencwang" && $req_event_info['type']==9)  ||	//sectionals chair
			($session->username == "viramete" && $req_event_info['type']==9)  ||	//special events coord
			($session->username == "garywu" && $req_event_info['type']==8)  ||		//creative director
			($session->username == "lee244" && $req_event_info['type']==6)  ||	//alumni liason
			...
			($session->username == "jutabha")	||									// webmaster 
			($session->username == "buranasi" && $req_event_info['type']==5) ||		//historian
			($session->username == "changarn" && ($req_event_info['type']==7 || $req_event_info['type']==0))  ||		//philanthropy cochair
			($session->username == "samantyl" && ($req_event_info['type']==7 || $req_event_info['type']==0))  ||		//phil cochair
			($session->username == "huan508" && ($req_event_info['type']==5 || $req_event_info['type']==8))  ||		//director of recruitment
			($session->username == "jinfungt" && ($req_event_info['type']==5 || $req_event_info['type']==8))  ||	//director of recruitment 
			($session->username == "pendergh" && $req_event_info['type']==10)  ||	//alpha fam head
			($session->username == "jseol" && $req_event_info['type']==10)  ||	//alpha fam head
			($session->username == "kaichinh" && $req_event_info['type']==11)  ||	//phi fam head
			($session->username == "tamhoang" && $req_event_info['type']==12)  || 	//omega fam head
			($session->username == "hayliewo" && $req_event_info['type']==5)  ||	//puncle
			($session->username == "yangvane" && $req_event_info['type']==5)  ||  		//puncle
			($session->username == "ayalam" && $req_event_info['type']==16)  		//Diversity & Inclusion
	) { 
	...
	while ($row = mysql_fetch_array($result)) {
		...
		if ($session->isOfficer() || $session->isFakeOfficer())  {	
			...
			($session->username == "ayalam" && $req_event_info['type']==16)  		//Diversity & Inclusion
				...
				if ($req_event_info['type'] == 0) {echo ("<a href=\"process.php?subflake=1&amp;event=".$req_event."&amp;user=".$row['username']."\" title=\"Flake\" onclick=\"return confirm('Are you sure you want to FLAKE ".$row['fname']." ".$row['lname']."?');\">&ndash;&frac12;</a>");} 
				else {echo ("<a href=\"process.php?subflakeother=1&amp;event=".$req_event."&amp;user=".$row['username']."\" title=\"Flake\" onclick=\"return confirm('Are you sure you want to FLAKE ".$row['fname']." ".$row['lname']."?');\">-1</a>");};
				...
				if($row['status'] == 1) { echo "<td><a href=\"process.php?sublead=1&amp;event=".$req_event."&amp;user=".$row['username']."&amp;lead=1\" title=\"Change Lead\" onclick=\"return confirm('Are you sure you want to take lead for this event?');\">Yes</a><br />
				<a href=\"process.php?sublead=0&amp;event=".$req_event."&amp;user=".$row['username']."&amp;lead=0\" title=\"Change Lead\" onclick=\"return confirm('Are you sure you want to NOT take lead for this event?');\">No</a></td>";} else {echo "<td></td>";};
...
if ($session->isOfficer() && $eventMax > 0 && mysql_num_rows($result) == $eventMax) {
	...
	if ($session->isOfficer() || $session->isFakeOfficer()) {	
		if (
			...
			if ($req_event_info['type'] == 0) {echo ("<a href=\"process.php?subflake=1&amp;event=".$req_event."&amp;user=".$row3['username']."\" title=\"Flake\" onclick=\"return confirm('Are you sure you want to FLAKE ".$row3['fname']." ".$row3['lname']."?');\">-&frac12;</a>");} else {echo ("<a href=\"process.php?subflakeother=1&amp;event=".$req_event."&amp;user=".$row3['username']."\" title=\"Flake\" onclick=\"return confirm('Are you sure you want to FLAKE ".$row3['fname']." ".$row3['lname']."?');\">-1</a>");};
			...
			?>
				...
				<fieldset>
					...
					<ol>
						...
						else {echo "<input type='hidden' id='txt_Drive' name='txt_Drive' value=0 />";};
						...
						else {echo "<input type='hidden' id='txt_Guest' name='txt_Guest' value=0 />";};
						...
						echo "<li><button type='submit' class='button'";
							if ($rowD['nAtt'] >= $req_event_info['max'] && $req_event_info['max'] != 0) {
								echo " onclick=\"return confirm('This event is currently full. If you sign up you will be added to the waitlist. You must be prepared to attend in the case that current attendees drop, or the cap is increased.');\">Sign Up for Wailitst";}
							else {
								# check if user wants to automatically create a Google Calendar event

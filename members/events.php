...
?>
	...
	// }					
					...
					// Fetch current user's information
						...
						else if(strcmp($_GET["span"],"soon") === 0)  {$dateSpan = "WHERE `end` >= DATE_SUB(NOW(), INTERVAL '10' HOUR) AND `end` <= DATE_ADD(CURDATE(), INTERVAL +14 DAY)";} // Note that this finds literally every upcoming event, even if it is in a future semester.
						...
						else {$dateSpan = "WHERE `end` >= DATE_SUB(NOW(), INTERVAL '10' HOUR)";}; // Similar to upcoming events -- this finds the next 14 days of events, no matter what semester they may be in.

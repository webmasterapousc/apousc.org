...
// Set values for page
$page_title = "Requirements :: Pledges";
...
<?php
	...
	$q = "SELECT S.username, U.fname, U.lname, U.family, SUM(CASE WHEN (E.type = 0 || E.type = 13) THEN S.weight*E.hours ELSE 0 END) AS totalSer, SUM(CASE WHEN E.type = 1 THEN weight ELSE 0 END) AS totalFel, SUM(CASE WHEN E.type = 2 THEN weight ELSE 0 END) AS totalFund, SUM(CASE WHEN (E.type = 3 || E.type = 13) THEN weight ELSE 0 END) AS totalInt, SUM(CASE WHEN (E.type = 4 || E.type = 6 || E.type = 10 || E.type = 11 || E.type = 12) THEN weight ELSE 0 END) AS totalMem FROM `" . TBL_SIGNUPS . "` AS S, `" . TBL_USERS . "` AS U, `" . TBL_EVENTS . "` AS E WHERE U.status = 1 AND E.end < NOW() AND S.eventid = E.ID AND S.username = U.username AND `start` >= '2014-01-12 00:00:00' GROUP BY S.username ORDER BY {$sortorder} {$direction}, {$sortorder2}";
